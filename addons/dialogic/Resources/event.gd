...
func get_property_translated(property_name:String) -> String:
	if !_translation_id.is_empty() and ProjectSettings.get_setting('dialogic/translation/enabled', false):
		#TranslationServer.set_locale("jp") #테스트용
		var base_key := get_property_translation_key(property_name)
		var nicknames: String = ""
		var timeline_name: String = ""
		timeline_name = dialogic.current_timeline.resource_path.get_file().get_basename()
		if event_name == "Text":
			if self.character:
				nicknames = self.character.nicknames[0]
		var bracketed_key := "%s (%s) (%s)" % [base_key, nicknames, timeline_name]
		var translation := tr(base_key)
		#print(self.character_identifier)
		
		# if no translation is found tr() returns the id, but we want to fallback to the original
		return translation if translation != base_key else _get_property_original_translation(property_name)
	else:
