...
var translator_lines: Array[PackedStringArray] = [] # 번역가용
...
func collect_lines_from_timeline(timeline: DialogicTimeline) -> void:
	var emotion_info: Dictionary = {}
	var line_count: int = 0
	
	for event: DialogicEvent in timeline.events:
		
		if event.event_name == "Emotion":
			if event.character:
				emotion_info[event.character.nicknames[0]] = event.emotion_identifier
			#print(emotion_info)

		if event is DialogicCharacterEvent:
			if event.action == event.Actions.LEAVE and event.character:
				emotion_info[event.character.nicknames[0]] = "Idle"
				#print(emotion_info)
		...
		if event.can_be_translated():
			...
			var speaker := ""
			var emotion := ""
			var stage_name := timeline.resource_path.get_file().get_basename()
			
			if "character" in event and event.character:
				speaker = event.character.nicknames[0]
				emotion = emotion_info.get(speaker, "Idle")
			...
			for property: String in properties:
				...
				# 기존 번역 CSV
				lines.append(PackedStringArray([
					line_key, line_value
				]))
				
				# 번역가용 CSV (메타정보 포함)
				translator_lines.append(PackedStringArray([
					line_key,      # 순수 Key
					speaker,       # 캐릭터
					emotion,       # 감정
					stage_name,    # 타임라인 이름
		...
		line_count += 1
	#print(timeline.resource_path.get_file().get_basename())
...
func update_csv_file_on_disk() -> void:
	...
	# 번역가용 CSV 저장
	var base_dir = used_file_path.get_base_dir()
	var translator_dir = base_dir.path_join(".gdignore")
	if not DirAccess.dir_exists_absolute(translator_dir):
		var dir = DirAccess.open(base_dir)
		if dir:
			dir.make_dir(".gdignore")

	var translator_path = translator_dir.path_join(
		used_file_path.get_file().get_basename() + "_translator.csv"
	)

	var tfile = FileAccess.open(translator_path, FileAccess.WRITE)
	tfile.store_csv_line(PackedStringArray(["Key", "Speaker", "Emotion", "Scene", "Source Text"]))
	for tline in translator_lines:
		tfile.store_csv_line(tline)
	tfile.close()
