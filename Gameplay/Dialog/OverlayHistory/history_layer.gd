...
func _ready() -> void:
	...
	get_show_history_button().modulate = Color.TRANSPARENT
	tween.tween_property(get_show_history_button(), "modulate", Color.WHITE, 1.0).from(Color.TRANSPARENT).set_delay(1.0)
